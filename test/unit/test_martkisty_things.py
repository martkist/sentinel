...
@pytest.fixture
def invalid_martkist_address(network='mainnet'):
...
def mn_list():
    ...
    masternodelist_full = {
        u'701854b26809343704ab31d1c45abc08f9f83c5c2bd503a9d5716ef3c0cda857-1': u'  ENABLED 70201 TFcpmdvna59VuWdhCQQYsKE4jm6tUMeuEJ 1474157572    82842 1474152618  71111 52.90.74.124:19999',
...
def test_valid_martkist_address():
    from martkistlib import is_valid_martkist_address

    main = valid_martkist_address()
    test = valid_martkist_address('testnet')
    main_btc = valid_bitcoin_address()
    test_btc = valid_bitcoin_address('testnet')

    assert is_valid_martkist_address(main) is True
    assert is_valid_martkist_address(main, 'mainnet') is True
    assert is_valid_martkist_address(main, 'testnet') is False

    assert is_valid_martkist_address(test) is False
    assert is_valid_martkist_address(test, 'mainnet') is False
    assert is_valid_martkist_address(test, 'testnet') is True

    assert is_valid_martkist_address(main_btc) is True
    assert is_valid_martkist_address(main_btc, 'mainnet') is True
    assert is_valid_martkist_address(main_btc, 'testnet') is False

    assert is_valid_martkist_address(test_btc) is False
    assert is_valid_martkist_address(test_btc, 'mainnet') is False
    assert is_valid_martkist_address(test_btc, 'testnet') is True


def test_invalid_martkist_address():
    from martkistlib import is_valid_martkist_address

    main = invalid_martkist_address()
    test = invalid_martkist_address('testnet')
    main_btc = invalid_bitcoin_address()
    test_btc = invalid_bitcoin_address('testnet')

    assert is_valid_martkist_address(main) is False
    assert is_valid_martkist_address(main, 'mainnet') is False
    assert is_valid_martkist_address(main, 'testnet') is False

    assert is_valid_martkist_address(test) is False
    assert is_valid_martkist_address(test, 'mainnet') is False
    assert is_valid_martkist_address(test, 'testnet') is False

    assert is_valid_martkist_address(main_btc) is False
    assert is_valid_martkist_address(main_btc, 'mainnet') is False
    assert is_valid_martkist_address(main_btc, 'testnet') is False

    assert is_valid_martkist_address(test_btc) is False
    assert is_valid_martkist_address(test_btc, 'mainnet') is False
    assert is_valid_martkist_address(test_btc, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from martkistlib import elect_mn
...
def test_parse_masternode_status_vin():
    from martkistlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % martkistlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(martkistlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(martkistlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(120).quantize(precision)
    assert int(martkistlib.blocks_to_seconds(16616)) == 996960
