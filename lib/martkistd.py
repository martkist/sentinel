...
class MartkistDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some martkistd methods
    ...
    @classmethod
    def from_martkist_conf(self, martkist_dot_conf):
        from martkist_config import MartkistConfig
        config_text = MartkistConfig.slurp_config_file(martkist_dot_conf)
        creds = MartkistConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = martkistlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = martkistlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = martkistlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_martkistd_proto_version_with_sentinel_ping)
