...
class GovernanceClass(object):
    ...
    def vote(self, martkistd, signal, outcome):
        return self.go.vote(martkistd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, martkistd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(martkistd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(martkistd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import martkistlib
        obj_data = martkistlib.SHIM_serialise_for_martkistd(self.serialise())
    ...
    def martkistd_serialise(self):
        import martkistlib
        return martkistlib.SHIM_serialise_for_martkistd(self.serialise())
